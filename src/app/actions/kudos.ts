...
const ToggleSchema = z.object({
  targetType: z.enum(["profile", "milestone"]),
...
export async function toggleKudos(input: { targetType: "profile" | "milestone"; targetId: string }): Promise<KudosResult> {
  ...
  if (recipient === actor.id) {
    // Self high-fives don't count. No-op.
    const [row] = await db
      .select({ id: kudos.id })
      .from(kudos)
      .where(
  ...
  if (parsed.targetType === "profile") {
    revalidatePath(`/directory/${parsed.targetId}`);
  }
...
async function totalCount(targetType: "profile" | "milestone", targetId: string) {
  const rows = await db
    .select({ id: kudos.id })
    .from(kudos)
